...
def worker(q, output_path, label_map_dict, exceptions, examples_number):
    task_popped = False
    ...
    while True:
        try:
            ...
            task_popped = False
            if idx % 100 == 0:
                logging.info('On image %d of %d', idx, examples_number)
...
def main():
  ...
  for i in range(threads_num):
    ...
    th = threading.Thread(target=worker, daemon=True, args=[q, output_path, label_map_dict, exceptions, len(examples_list)])
    threads.append(th)
    th.start()
